# resbooking/framework

MVC, security, background tasks and migrations framework.
...
```bash
composer require resbooking/framework
```
...
```json
{
    ...
    "require": {
        "resbooking/framework": "@dev"
    }
}
```

Then run `composer update resbooking/framework`.

## Documentation

See the [docs](docs/) folder or the main repository [docs/framework](https://github.com/aseguragonzalez/resbooking/tree/main/docs/framework) for architecture and usage.
...
3. The release workflow runs automatically. If the tag `framework-v{VERSION}` does not exist, it validates, builds, and publishes a GitHub Release with the zip artifact.
