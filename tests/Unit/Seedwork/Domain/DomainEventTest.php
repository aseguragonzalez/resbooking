...
class DomainEventTest extends TestCase
{
    ...
    {
        $event = new DummyDomainEvent(EntityId::fromString('my-id'));
        $this->assertSame('my-id', $event->id->value);
    }

    public function testTypePropertyReturnsType(): void
    {
        $event = new DummyDomainEvent(EntityId::fromString('id'), 'MyType');
        $this->assertSame('MyType', $event->type);
    }

    public function testVersionPropertyReturnsVersion(): void
    {
        $event = new DummyDomainEvent(EntityId::fromString('id'), 'type', '3.0');
    ...
    {
        $payload = ['key' => 'value'];
        $event = new DummyDomainEvent(EntityId::fromString('id'), 'type', '1.0', $payload);
    ...
    public function testCreatedAtPropertyReturnsDateTime(): void
    {
        ...
        $event = new DummyDomainEvent(EntityId::fromString('id'), 'type', '1.0', [], $date);
        $this->assertEquals($date, $event->createdAt);
    }

    public function testEqualsReturnsTrueForSameId(): void
    {
        $event1 = new DummyDomainEvent(EntityId::fromString('same-id'));
        $event2 = new DummyDomainEvent(EntityId::fromString('same-id'));
        $this->assertTrue($event1->equals($event2));
    }

    public function testEqualsReturnsFalseForDifferentId(): void
    {
        $event1 = new DummyDomainEvent(EntityId::fromString('id-1'));
        $event2 = new DummyDomainEvent(EntityId::fromString('id-2'));
