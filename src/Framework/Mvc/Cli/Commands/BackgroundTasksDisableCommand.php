...
final class BackgroundTasksDisableCommand implements Command
{
    ...
    {
        return 'background-tasks:disable';
    }

    public function getDescription(): string
    {
        return 'Disable background tasks in mvc.config.json and add migration to drop default table';
    }
    ...
    public function execute(array $args): int
    {
        ...
        if (!$config->isBackgroundTasksEnabled()) {
            $this->output->info('Background tasks are already disabled in ' . MvcConfig::CONFIG_FILENAME . '.');
            return 0;
        }
        ...
        if (!$skipMigrations) {
            ...
            if ($leaf === null) {
                $this->output->error(
                    'Cannot create background tasks teardown migration: migrations module not found or disabled. '
            ...
            try {
                $migrationDir = BackgroundTasksDefaultMigrationWriter::createDisableMigration($leaf);
            } catch (\Throwable $e) {
        ...
        $this->output->success('Background tasks disabled in ' . MvcConfig::CONFIG_FILENAME . '.');
        if (!$skipMigrations) {
    ...
    {
        $this->output->line('Usage: mvc background-tasks:disable [--path=<app-path>] [--skip-migrations]');
        $this->output->line();
