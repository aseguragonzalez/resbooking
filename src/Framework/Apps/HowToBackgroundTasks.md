...
Without `--skip-migrations`, the CLI creates a timestamped folder under your migration module’s `migrations/` directory. The forward script creates the `background_tasks` table expected by `Framework\BackgroundTasks\Infrastructure\SqlTaskRepository`.
...
2. Register a `Framework\BackgroundTasks\TaskHandlerClassMap` (task type string → `TaskHandler` class name) for your port’s handlers.
3. Register logging and any other app-specific services.
4. Call `Framework\BackgroundTasks\Dependencies::configure($container)` to wire the default SQL task repository, transaction runner, and container-backed `TaskHandlerRegistry`.

Edit that bootstrap class if you need different configuration sources or custom `TaskRepository` wiring (see above).

The runnable class is `Framework\BackgroundTasks\BackgroundTasksApp`: it only processes batches; it does not read environment variables itself.
...
- [How to enable MVC authentication](../Module/HowToAuthentication.md) — similar enable/disable + migration pattern.
