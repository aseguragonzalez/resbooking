...
{
    private const TASK_TYPE = 'send_reset_password_challenge_email';
    ...
    public static function fromTask(Task $task): self
    {
        ...
        if ($invalidArguments !== []) {
            throw new \InvalidArgumentException(sprintf(
                'Invalid task arguments for reset-password challenge email. Task id: "%s", invalid: %s',
                $task->id,
        ...
        if (!is_string($expiresAt) || $expiresAt === '') {
            throw new \InvalidArgumentException(sprintf(
                'Missing or invalid "expiresAt" in task arguments for reset-password email. Task id: "%s"',
                $task->id
            ));
        }
