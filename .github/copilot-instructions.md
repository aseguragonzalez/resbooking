...
- Prefer **stubs** over mocks when the test only needs a dependency to return a
 value (no need to verify call count or arguments). Use `createStub(Interface::class)`
 and `->method('...')->willReturn(...)`; do **not** use `#[AllowMockObjectsWithoutExpectations]`.
- Use **mocks** (e.g. `createMock()` with `expects($this->once())` or
 `expects($this->never())`) only when the test must verify that a method was
 called (or not called) or was called with specific arguments.
